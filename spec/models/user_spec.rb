...
describe User do
  ...
  it 'resets password' do
    users(:data1).update_attributes(:password => 'new password', :password_confirmation => 'new password')
    User.authenticate('koto', 'new password').should == users(:data1)
  end

  it 'does not rehash password' do
    users(:data1).update_attributes(:login => 'quentin2')
    User.authenticate('quentin2', 'tteesstt').should == users(:data1)
  end
  ...
  it 'authenticates user' do
    User.authenticate('koto', 'tteesstt').should == users(:data1)
  end

  it "doesn't authenticate user with bad password" do
    User.authenticate('koto', 'invalid_password').should be_nil
  end
 ...
 if REST_AUTH_SITE_KEY.blank?
  ...
  it 'sets remember token' do
    users(:data1).remember_me
    users(:data1).remember_token.should_not be_nil
    users(:data1).remember_token_expires_at.should_not be_nil
  end

  it 'unsets remember token' do
    users(:data1).remember_me
    users(:data1).remember_token.should_not be_nil
    users(:data1).forget_me
    users(:data1).remember_token.should be_nil
  end

  it 'remembers me for one week' do
    before = 1.week.from_now.utc
    users(:data1).remember_me_for 1.week
    after = 1.week.from_now.utc
    users(:data1).remember_token.should_not be_nil
    users(:data1).remember_token_expires_at.should_not be_nil
    users(:data1).remember_token_expires_at.between?(before, after).should be_true
  end

  it 'remembers me until one week' do
    time = 1.week.from_now.utc
    users(:data1).remember_me_until time
    users(:data1).remember_token.should_not be_nil
    users(:data1).remember_token_expires_at.should_not be_nil
    users(:data1).remember_token_expires_at.should == time
  end
