...
describe BelongsController do
  ...
  def mock_belong(stubs={})
    @mock_belong ||= mock_model(Belong, stubs)
  end

  describe "GET index" do
    it "assigns all belongs as @belongs" do
      Belong.stub!(:find).with(:all).and_return([mock_belong])
      get :index
      assigns[:belongs].should == [mock_belong]
    end
  end

  describe "GET show" do
    it "assigns the requested belong as @belong" do
      Belong.stub!(:find).with("37").and_return(mock_belong)
      get :show, :id => "37"
      assigns[:belong].should equal(mock_belong)
    end
  end

  describe "GET new" do
    it "assigns a new belong as @belong" do
      Belong.stub!(:new).and_return(mock_belong)
      get :new
      assigns[:belong].should equal(mock_belong)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created belong as @belong" do
        Belong.stub!(:new).with({'these' => 'params'}).and_return(mock_belong(:save => true))
        post :create, :belong => {:these => 'params'}
        assigns[:belong].should equal(mock_belong)
      end

      it "redirects to the created belong" do
        Belong.stub!(:new).and_return(mock_belong(:save => true))
        post :create, :belong => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved belong as @belong" do
        Belong.stub!(:new).with({'these' => 'params'}).and_return(mock_belong(:save => false))
        post :create, :belong => {:these => 'params'}
        assigns[:belong].should equal(mock_belong)
      end

      it "re-renders the 'new' template" do
        Belong.stub!(:new).and_return(mock_belong(:save => false))
        post :create, :belong => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested belong" do
        Belong.should_receive(:find).with("37").and_return(mock_belong)
        mock_belong.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :belong => {:these => 'params'}
      end

      it "assigns the requested belong as @belong" do
        Belong.stub!(:find).and_return(mock_belong(:update_attributes => true))
        put :update, :id => "1"
        assigns[:belong].should equal(mock_belong)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the belong as @belong" do
        Belong.stub!(:find).and_return(mock_belong(:update_attributes => false))
        put :update, :id => "1"
        assigns[:belong].should equal(mock_belong)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the belongs list" do
      Belong.stub!(:find).and_return(mock_belong(:destroy => true))
      delete :destroy, :id => "1"
